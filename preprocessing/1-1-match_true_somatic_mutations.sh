...
while IFS=' ' read -r file_id tcga_id; do
  sample_dir="$save_dir/$tcga_id"
  mkdir -p "$sample_dir"

  zcat "${data_dir}/tcga/true_somatic_mutations/${file_id}"/*.maf.gz | grep -v '^#' | sed '1d' | awk '{ print $5 "_" $6 }' > "$sample_dir/tcga_true_variants.txt"
