...
## Select samples with somatic number > 20
...
model_name = "xgboost-SMOTE_Filter-num20"
...
cases_info = pd.read_csv("/gpfs/commons/groups/gursoy_lab/xli/somatic_variant_prediction/tcga_info/cases_info.txt", sep='\t')
disease_df = cases_info[['submitter_id', 'disease_type']]
...
num_somatic = combined_df.groupby('sample_id')['if_true_somatic'].sum()
select_samples = num_somatic[num_somatic > 20].index.values

combined_df = combined_df[combined_df['sample_id'].isin(select_samples)]
